...
本目录保存 V1 hicode Prompt 模板，用于约束 Coding Agent 在需求评审、编码计划、TDD、辅助编码、代码审查、提交检查、核心场景测试和发布检查中的输入、处理规则、安全边界和输出格式。

- 本仓库产出路径：`references/prompts/`
- 目标项目落地路径：`.hicode/prompts/`
- 维护对象：AI 工程化小组、研发负责人、质量负责人
- 主要使用者：开发人员、Coding Agent、节点门禁 Agent
- 更新时机：流程变化、质量规则变化、安全规则变化、Prompt 使用复盘后

Prompt 是 Agent 工作的执行入口之一，但不替代 `AGENTS.md`、`docs/` 上下文文档、人工 Review 或发布审批。本仓库中先使用可见源目录维护资产，后续由安装脚本把 Prompt 放入目标项目的 `.hicode/prompts/`。

## 2. 设计原则

1. 中文为主，英文为辅；文件名、路径、代码标识、协议名和通用技术术语可保留英文。
2. Prompt 必须绑定具体研发场景，不写泛化、不可验收的万能 Prompt。
3. Prompt 必须明确目标、上下文、输入、检查维度、输出格式和约束，避免只给一句自然语言指令。
...
8. Prompt 默认服务保险/金融核心系统研发，必须按金融核心系统风险标准处理保险核心业务逻辑严谨性、金额精度、交易一致性、状态流转、幂等、权限、审计、客户隐私、监管合规、生产变更、回滚和发布准入。
9. Prompt 输出必须要求验证依据和局限，不把一次有效输出当作普适可靠结果。
...
| 辅助编码 | `coding-assistant.md` | 引用 `docs/workflows/coding-plan.md` 和 `docs/workflows/tdd.md` | `skills/coding-assistant/` |
...
P3-WP3 已确认 V1 不单独维护 `coding-assistant.md` 的独立 workflow；辅助编码 Prompt 应引用编码计划、TDD 和编码规范，后续如试点复盘发现流程复杂度升高，再在 P4 或后续工作中补充独立 workflow。

## 4. 标准结构

每个 Prompt 的主干统一采用以下骨架。文件名、workflow、Skill 和目标安装路径等元信息可以放在标题下的简短表格中，但不得干扰主执行结构。

1. `角色`：Agent 应扮演的研发职责，不写超出职责的身份。
2. `任务目标`：本次 Prompt 要完成的具体任务、服务的研发流程节点、适用场景和不适用场景。
3. `必须读取的上下文`：执行前必须读取的文档、输入或缺失时的处理方式。
4. `输入材料`：用户或工具需要提供的信息。
5. `检查维度`：Agent 的分析、检查、生成、验证维度和本场景质量标准。
6. `输出要求`：固定 Markdown 或 JSON 输出格式，并要求说明依据、风险、建议动作、待确认问题和验证动作。
7. `约束`：不可读取、不可输出、不可执行的行为，以及不确定内容处理规则。
8. `上下文更新`：需要补充到目标项目 `docs/`、目标项目 `.hicode/`，或本仓库 hicode 根目录源资产的内容。

十二段式中的 `适用场景`、`处理规则`、`安全约束`、`输出格式`、`质量标准`、`待确认问题`、`上下文更新建议` 和 `禁止事项` 不作为 V1 顶层章节单独展开，应分别收敛到八段式对应章节中：
...
上下文更新位置使用当前仓库统一命名：`docs/DOMAIN_KNOWLEDGE.md`、`docs/PROJ_CONTEXT.md`、`docs/PRD_CONTEXT.md`、`docs/DEFECT_CASES.md` 和 `docs/ADR/`。不要使用 `docs/CONTEXT.md` 或 `docs/PRD-CONTEXT.md` 指代目标项目上下文。
...
所有 Prompt 都应默认按保险/金融核心系统服务对象设计。除非具体场景明确不适用，否则处理规则、风险清单和质量标准必须覆盖：

1. 保险核心业务逻辑严谨性：投保、批改、退保、核保、理赔前置、收付费、责任判断、保单/批单状态、期间、费率、责任、权益和规则例外。
2. 金额精度：金额类型、舍入、币种、费率、保费、赔付、冲正和对账。
3. 交易一致性：事务边界、跨系统调用、补偿、重试、最终一致性和异常恢复。
4. 状态流转：保单、批单、理赔、支付、清算等状态是否可达、可逆、可追踪。
5. 幂等与并发：重复请求、重复回调、并发修改、锁、版本号和防重。
6. 权限与审计：操作权限、审批链路、操作留痕、审计字段和责任人。
7. 隐私与监管：客户敏感信息、日志脱敏、数据最小化、监管报送和合规留痕。
8. 生产变更与发布：配置、SQL、脚本、回滚、生产验证点和发布准入。

若某个维度缺少输入，应输出待确认问题，不得编造业务规则。
...
4. 能覆盖关键安全红线。
5. 能覆盖金融核心系统检查基线，特别是保险核心业务逻辑严谨性。
6. 能产出可用于人工 Review、提交检查、测试补充或发布审批的材料。
7. 不复制大段需求正文，只保留必要规则和引用。
8. 能说明本次输出的验证依据、未验证部分和需要人工确认的边界。
