...
**hicode 规则按需读取边界**:
`references/rules/` 是当前规则源，但不随 plugin 默认全量加载。`hi` 总入口默认只做诊断和路由；`init`、`scope`、`tdd`、`review` 和 `release` 只读取自身场景规则和必要的 `shared/` 规则；Agent 只读取委托任务相关规则。`shared/` 必须保持短小，避免重新变成大杂烩。
...
**hicode 初始化引导边界**:
`hi` Skill 默认只诊断目标项目初始化状态、整理初始化输入并路由到 `hicode:init`；只有用户明确要求初始化并确认写入范围后，才由 `init` 协助生成或修改目标项目 `AGENTS.md`、`CLAUDE.md`、`CONTEXT.md`、`docs/PROJ_CONTEXT.md` 或规则文档。
