...
SkillOpt 首批评估训练使用的人工构造样例集合，包含脱敏 diff、需求证据、TDD 证据、配置/SQL/Java/保险核心风险线索、期望识别点、风险等级和禁止结论。它用于稳定 evaluator 和验证门禁，不直接采集真实会话、真实 MR、真实项目代码、生产日志、生产配置、客户信息或密钥。
...
SkillOpt 产出的候选 `best_skill.md` 或优化建议只能作为人工审查输入，必须先生成候选差异摘要，确认不冲突于 hicode 安全边界、V3 资产边界、中文文档风格、建议性输出枚举、Review 三轴审查和金融核心风险标准后，才能通过普通代码修改流程合入运行 Skill，并复跑健康检查和 held-out eval。
