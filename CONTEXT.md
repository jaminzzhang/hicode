...
安装到 Coding Agent 平台的 hicode 插件资产，用于让 Claude Code、OpenCode 等 Agent 平台获得 hicode 的命令、入口或后续初始化能力。该插件不等同于目标项目 `.hicode/` 运行目录，不在安装阶段扫描目标项目代码，不生成 `CLAUDE.md`、`AGENTS.md` 或项目上下文。
_Avoid_: 目标项目初始化结果、业务项目代码扫描器、生产环境插件、把平台插件安装和项目初始化混为一步

**hicode Plugin 安装器**:
`harness-assets/plugins/install.sh`，用于把 hicode Coding Agent Plugin 安装到 Claude Code、OpenCode 等 Coding Agent 平台。安装器只处理平台插件安装或注册，不执行目标项目初始化、不扫描代码、不生成项目级入口文件或 `.hicode/` 运行资产。
