...
**Agent 整合规范边界**:
`harness-assets/docs/AGENT_PROMPT_INTEGRATION.md` 与 `harness-assets/docs/workflows/agent-delegation.md` 用于定义 Agent、Prompt、Skill、门禁、Schema 和输出模板之间的职责关系、引用关系、委托流程、冲突优先级和降级口径。该整合规范不更新目标项目入口模板，不创建初始化清单，不设计 Hook，不新增 Prompt、Skill、门禁或 Schema。
...
目标项目初始化 hicode 资产时的上下文分层策略。`DAILY` 表示应随目标项目日常会话默认可用的入口、当前技术栈规则、核心 Prompt/Skill/Agent 和关键门禁；`LIBRARY` 表示保留为可检索、可按需调用的参考资产、专项规则、示例和扩展能力。该策略用于降低上下文噪音和误触发风险，不表示删除或降低资产重要性。
_Avoid_: 全量资产每次默认加载、把 LIBRARY 资产删除、凭偏好分类而不引用目标项目证据

**hicode 初始化规划目录**:
`harness-assets/init/` 是后续维护目标项目初始化和加载清单的源资产目录，用于描述 Agent、Prompt、Skill、门禁、Hook、Schema、文档和示例如何从 `harness-assets/` 初始化到目标项目 `.hicode/` 或目标项目文档目录。该目录可承载 profile、manifest 和 `DAILY/LIBRARY` 加载分层，不放真实运行副本、不替代目标项目 `.hicode/`。
_Avoid_: 把初始化清单放进人读 `docs/`、在本仓库维护 `harness-assets/.hicode/`、把初始化规划写成真实初始化结果
...
**hicode profile**:
hicode 选择性初始化中的资产组合，用于表达某类目标项目应初始化或保留哪些 manifest 条目。`core` 表示完整轻量闭环；`java-insurance-core` 表示 Java 保险核心系统默认组合；`full-library` 表示全量可检索资产库。profile 不应随意改写 manifest 条目的加载分层。
_Avoid_: 把 core 缩减为只有入口文件、把 full-library 理解为全量默认加载、让 profile 覆盖资产原始 load_tier 语义

**hicode Coding Agent Plugin**:
当前第一版指安装到 Claude Code 的 hicode 插件资产，用于让 Claude Code 获得 hicode 的命令、入口、能力场景 Skill 或后续初始化指引。该插件不等同于目标项目 `.hicode/` 运行目录，不在安装阶段扫描目标项目代码，不生成 `CLAUDE.md`、`AGENTS.md` 或项目上下文。OpenCode plugin 机制与 Claude Code plugin 机制不同，后续若适配应独立设计，不混入第一版 Claude Code plugin root。
_Avoid_: 目标项目初始化结果、业务项目代码扫描器、生产环境插件、把平台插件安装和项目初始化混为一步

**hicode plugin 内部分层**:
当前第一版对外只维护一个 Claude Code 规范的 `hicode plugin`，以 `harness-assets/plugins/` 作为 plugin root；内部再区分入口层和能力层。入口层提供 hicode 定位、安全边界和后续初始化入口；能力层选择性打包稳定的 Harness 资产，让 Claude Code 可直接调用 hicode Skill、Agent、规则和流程入口。OpenCode plugin 机制与 Claude Code plugin 机制不同，不纳入第一版同一目录模型。
_Avoid_: 同时混用 Claude Code 与 OpenCode 的 plugin 规范、拆成多个对外 plugin 增加安装心智、替代目标项目初始化、全量默认加载所有资产、绕过人工确认或门禁、自动合并发布或操作生产

**hicode plugin 能力场景包**:
hicode plugin 第一版能力层按能力场景打包，而不是按 `harness-assets/` 目录全量搬运。首批能力场景包括需求到编码前链路、TDD 与辅助编码链路、Review 与提交链路、发布与回归链路。每个场景只选择稳定、可直接调用、低噪音且能保留安全红线的 Agent、Skill、规则和流程入口。
_Avoid_: 把 docs/prompts/skills/gates/schemas/examples 整目录默认塞入平台插件、让平台插件替代目标项目初始化、把低频模板和回归样例默认加载进每次会话

**hicode Plugin 安装器**:
`harness-assets/plugins/install.sh`，用于把 hicode Coding Agent Plugin 安装到当前用户的 Claude Code。安装器只处理 Claude Code plugin 安装或注册，不执行目标项目初始化、不扫描代码、不生成项目级入口文件或 `.hicode/` 运行资产。
_Avoid_: 目标项目初始化脚本、代码扫描脚本、生产发布脚本、自动修改业务仓库的工具

**目标项目 Harness 运行目录**:
目标项目中的 `.hicode/`，由后续初始化流程或初始化脚本根据 `harness-assets/` 下的可见源目录生成或复制，不在本仓库作为源产出目录直接维护。
...
**大型代码库 Agent 工作方法论**:
面向多模块单体、Monorepo、多仓服务群或长期遗留系统的 hicode Agent 使用方法，强调实时代码导航、分层上下文、按需 Skill/Agent、局部命令、Hook 边界、LSP/MCP 工具边界和治理 Owner。该方法论沉淀在 `harness-assets/docs/LARGE_CODEBASE_AGENT_GUIDE.md`，作为按需读取的指导资产，不替代入口规则、项目上下文、Prompt、Skill、门禁或人工负责人确认。
...
**V2 演进边界**:
ECC 对标后确认的 `harness-assets/agents/`、Agent 与 Prompt 整合、`DAILY/LIBRARY` 选择性初始化、`harness-assets/init/` 和门禁 Hook 化属于 V2 演进方向，不回改 V1 已完成工作包状态。V1 作为已完成基线保留；V2 应另行形成规划、工作包和验收口径。
