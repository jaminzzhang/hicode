...
**Harness 运行资产源目录**:
`agents/`、`references/prompts/`、`references/skills/`、`references/gates/`、`references/schemas/` 和 `references/examples/` 等可见目录，用于维护未来会安装到目标项目 `.hicode/` 下的子 Agent、Prompt、Skill、门禁、Schema 和示例资产。
...
**hicode 初始化规划目录**:
`references/init/` 是后续维护目标项目初始化和加载清单的源资产目录，用于描述 Agent、Prompt、Skill、门禁、Hook、Schema、文档和示例如何从 hicode 根目录源资产初始化到目标项目 `.hicode/` 或目标项目文档目录。该目录可承载 profile、manifest 和 `DAILY/LIBRARY` 加载分层，不放真实运行副本、不替代目标项目 `.hicode/`，也不由 plugin 安装动作自动执行。
...
**hicode plugin 能力场景包**:
hicode plugin 第一版能力层按能力场景打包，而不是把 `references/` 全量作为默认上下文。首批能力场景包括需求到编码前链路、TDD 与辅助编码链路、Review 与提交链路、发布与回归链路。每个场景只选择稳定、可直接调用、低噪音且能保留安全红线的 Agent、Skill、规则和流程入口。
...
**目标项目 Harness 运行目录**:
目标项目中的 `.hicode/`，由后续初始化流程或初始化脚本根据 hicode 根目录源资产和 `references/init/` 规划生成或复制，不在本仓库作为源产出目录直接维护。
...
**Prompt 模板库**:
`references/prompts/` 下的 V1 场景化 Prompt 集合，用于约束 Agent 在需求、编码、测试、Review、提交和发布检查中的输入、处理规则、安全边界和输出格式；未来安装到目标项目后位于 `.hicode/prompts/`。
...
**`.hicode/`**:
单独写 `.hicode/` 时默认指目标项目安装后的 Harness 运行目录；本仓库源产出必须使用 `references/prompts/`、`references/skills/`、`references/gates/`、`references/schemas/` 等可见目录。
