...
**目标项目 hicode 入口补充片段**:
重构后由 `references/templates/project/hicode-entry-section.md` 承载，用于补充到目标项目已有、由 Agent 可执行初始化能力生成或由用户手工初始化生成的 `AGENTS.md` 或 `CLAUDE.md`。非 Claude Code 平台主要使用 `AGENTS.md`，Claude Code 使用 `CLAUDE.md`；hicode 不再维护完整入口模板，只维护文档路径、单需求文档生命周期、Skill 路由、规则目录和安全边界等平台无关补充片段。旧 `references/target-project/` 不再作为当前有效目录。
...
**hicode 根目录资产分层**:
根目录只把 Claude Code 可直接识别或 Coding Agent 会直接调用的入口资产做成一等目录，其中 `skills/` 是直接可调用 Skill，`skills/init/coding_rules.md` 是 `hicode:init` 目标项目规则种子，`skills/<skill>/` 是各 Skill 具体模板文档的平铺目录，`agents/` 是子 Agent，`references/` 只承载当前 `rules/`、`templates/` 和 `hooks/` 三类源资产，`archive/` 保存历史追溯材料。
...
_Avoid_: 非 init Skill 必读本仓库 `coding_rules.md`、Skill 直接读取 `references/`、把模板当成目标项目事实、在非 init 场景读取项目级模板替代 `docs/DOMAIN_KNOWLEDGE.md` 或 `docs/PROJ_CONTEXT.md`、每个场景 Skill 维护重复生命周期 README
...
_Avoid_: Agent 依赖共享运行目录、把模板当成目标项目事实、让 Agent 读取 `references/` 作为运行默认上下文

**hicode 当前有效资产**:
hicode 当前执行、安装和 Skill/Agent 路由可引用的资产集合。重构后应只包括根目录 `skills/`、`skills/init/`、各 Skill 根目录具体模板文档、`agents/` 以及 `references/rules/`、`references/templates/`、`references/hooks/` 等被明确保留的当前目录；归档资产不得再作为当前 Skill 的执行依据或默认检索来源。目标项目执行非 init Skill 时优先读取目标项目入口规则、`docs/rules/` 和本 Skill 根目录中的模板文档，避免跨 plugin 运行资产边界读取 `references/`。
...
**hicode 规则按需读取边界**:
`references/rules/` 是当前规则维护源，但不随 plugin 默认全量加载。`hicode:init` 的运行时种子副本位于 `skills/init/coding_rules.md`，用于创建或更新目标项目 `docs/rules/`；`scope`、`tdd`、`review`、`release` 在目标项目执行时只读取并遵守目标项目规则文件。专业 Agent 的共性安全、权限、输出和停止条件写入各 Agent 正文。Hook 说明和维护文档可以引用源文件 `references/rules/coding_rules.md` 作为维护依据。
...
**单需求文档生命周期规则**:
`references/templates/project/hicode-entry-section.md` 中集中维护的单需求文档创建、更新、缺失材料处理、证据降级和阶段交接规则；`hicode:init` 将其写入目标项目 `AGENTS.md` 或 `CLAUDE.md`。`scope`、`tdd`、`review` 和 `release` 只保留本阶段动作，并在目标项目执行时读取目标项目入口中的 hicode section。
_Avoid_: 每个 Skill 复制一套 `docs/features/<feature-id>/` 写入规则、缺失材料处理口径不一致、把阶段报告写成审批结论、在场景 Skill 下维护重复 `README.md`

**hicode 项目模板加载边界**:
`references/` 中的项目模板是维护源，不随 plugin 默认加载，也不在安装时自动写入目标项目；`hicode:init` 需要的项目模板副本放在 `skills/init/`。当前 `hicode:init` 初始化入口文件时优先使用 Agent 可执行初始化能力；若目标平台只有用户手工 TUI `/init`，例如 OpenCode，则 Agent 不得声称已调用 `/init`，应提示用户可手工执行，并把“确认后生成最小 `AGENTS.md`”作为推荐继续路径。入口文件生成或存在后，hicode 只基于 `hicode-entry-section.md` 补充 hicode section，包含文档路径、单需求生命周期、Skill 路由、规则目录和安全边界。项目模板只保留为可追溯、可复制填写的支撑材料，是否使用必须由用户明确确认。
...
**hicode 初始化引导边界**:
`hi` Skill 默认只诊断目标项目初始化状态、整理初始化输入并路由到 `hicode:init`。`init` 采用引导式初始化，遇到不确定或需要用户决策时一次只问一个问题；目标项目入口文件缺失时优先使用当前 Agent 可执行的初始化能力生成 `AGENTS.md` 或 `CLAUDE.md`。OpenCode `/init` 属于 TUI 内置命令，Agent 不能直接调用时提示用户可手工执行，并推荐在用户确认后生成最小入口文件。入口主体生成或存在后，hicode 只基于 `references/templates/project/hicode-entry-section.md` 补充 hicode section。
...
**hicode 项目规则文档拆分策略**:
旧 `references/docs/` 中的项目规则文档，例如编码规范、测试指南、Review 规则和发布指南，迁移时应拆成两类：面向目标项目复制填写的模板版进入 `references/templates/`；hicode 当前执行需要的通用规则进入根目录 Skill、专业 Agent 或 `references/rules/coding_rules.md`。原文件进入 `archive/` 或拆解后移除当前路径，避免同一文件同时承担目标项目模板和 hicode 执行规则两种职责。
...
**目标项目入口轻量路由**:
`references/templates/project/hicode-entry-section.md` 是目标项目入口文件的 hicode 补充片段，只维护 hicode 文档路径、单需求文档生命周期、核心流程路由、安全边界和输出规范。目标项目入口主体优先由当前 Agent 可执行初始化能力生成；OpenCode TUI `/init` 等用户手工命令不能由 Agent 代替执行。hicode 不再维护完整 `AGENTS.md` 或 `CLAUDE.md` 模板，也不复制完整规则、降级表、冲突优先级或规则全文。
...
**`docs/`**:
单独写 `docs/` 时默认指根目录项目管理文档目录；目标项目可复制填写的文档模板必须明确写作 `references/`。
