...
**目标项目 Agent 入口模板**:
重构后由 `references/templates/project/AGENTS.md` 和 `references/templates/project/CLAUDE.md` 承载，用于作为未来试点仓库或目标项目可复用的 Coding Agent 第一入口模板。`AGENTS.md` 面向 Codex、OpenCode 和其他使用 AGENTS 约定的平台；`CLAUDE.md` 面向 Claude Code。入口模板应作为项目入口、上下文索引、工作流程路由、安全边界、Skill 调用说明和输出规范，不作为完整领域知识库。旧 `references/target-project/` 不再作为当前有效目录。
...
**hicode 根目录资产分层**:
根目录只把 Claude Code 可直接识别或 Coding Agent 会直接调用的入口资产做成一等目录，其中 `skills/` 是直接可调用 Skill，`agents/` 是子 Agent，`references/` 只承载当前 `rules/`、`templates/` 和 `hooks/` 三类按需支撑资产，`archive/` 保存历史追溯材料。
_Avoid_: 把 prompts、gates、schemas 和 docs 全部平铺到根目录、让 Skill 只引用旧 `harness-assets/`、把 references 当成默认全量上下文、把 archive 当成当前规则源

**hicode 当前有效资产**:
hicode 当前执行、安装和 Skill/Agent 路由可引用的资产集合。重构后应只包括根目录 `skills/`、`agents/` 以及 `references/rules/`、`references/templates/`、`references/hooks/` 等被明确保留的当前目录；归档资产不得再作为当前 Skill 的执行依据或默认检索来源。
...
**hicode 统一规则目录**:
`references/rules/`，用于维护当前有效的规则、流程、门禁、Review 细则、结构化输出约束和 Skill/Agent 可引用的执行细则。重构后它应吸收原 `references/prompts/`、`references/gates/`、`references/schemas/`、`references/docs/review-rules/` 和 `references/docs/workflows/` 中仍然有效的规则性内容，并按 Skill 场景组织：`shared/`、`init/`、`scope/`、`tdd/`、`review/` 和 `release/`。根目录 Skill 可以按场景读取该目录下的规则，但不得把责任转交给历史归档资产。
_Avoid_: 多套规则源并行、Prompt/Gate/Schema 独立一级目录继续承载当前规则、按旧资产类型换皮保留复杂度、规则散落在模板或归档区

**hicode 规则按需读取边界**:
`references/rules/` 是当前规则源，但不随 plugin 默认全量加载。`hicode` 总入口默认只做诊断和路由；`init`、`scope`、`tdd`、`review` 和 `release` 只读取自身场景规则和必要的 `shared/` 规则；Agent 只读取委托任务相关规则。`shared/` 必须保持短小，避免重新变成大杂烩。
_Avoid_: 默认读取全部 rules、每个 Skill 都加载所有场景规则、Agent 为单一专项任务读取全库规则、shared 承载长篇指南或历史材料

**hicode Markdown 结构化输出规则**:
当前用于约束 Skill、Agent、Review 和门禁类输出的 Markdown 规则，记录固定字段、稳定枚举、风险等级、建议结论、问题列表、证据记录和未执行验证原因。旧 JSON Schema 文件不再作为当前有效资产；迁移时应归档原 JSON，并把仍有效的字段和枚举压缩进 `references/rules/shared/` 或对应场景规则。
_Avoid_: 当前 Skill 读取 JSON Schema、为未接入自动校验的结构保留独立 JSON、每个场景自定义冲突字段、把建议结论写成审批状态

**hicode 模板目录**:
`references/templates/`，用于维护目标项目当前可复制、可填写、可落地的模板，例如 ADR、PRD 上下文、项目上下文、领域知识、项目规则、Review/发布/门禁报告和输出模板。模板目录只保存结构化填写骨架，不承载执行规则全文；执行规则应放入 `references/rules/`。重构后模板按 `project/` 加 Skill 场景组织：`project/` 保存目标项目基础模板，`scope/`、`tdd/`、`review/` 和 `release/` 保存各场景输出模板。本仓库历史验收清单、试点运营模板、回归样例、旧初始化 manifest/profile 和过去工作包材料应进入 `archive/`。
_Avoid_: 把模板写成规则库、把执行流程藏在模板里、让模板替代 Skill 或 Gate 规则、把历史建设过程模板暴露为当前目标项目模板、按旧 docs/prompts/examples 类型散放模板

**hicode 项目模板加载边界**:
`references/templates/project/` 中的目标项目模板不随 plugin 默认加载，也不在安装时自动写入目标项目。只有用户明确进入 `hicode:init` 并确认目标项目写入范围后，`skills/init/SKILL.md` 才能按需读取这些模板，生成或更新目标项目入口、上下文和项目规则文档。
_Avoid_: plugin 安装时加载项目模板、非 init 任务默认读取项目模板、无确认写入目标项目、把项目模板等同于本仓库 `docs/`
...
**hicode 场景 Skill**:
根目录 `skills/` 下直接维护的 Claude Code 可调用 Skill，当前固定保留 6 个入口：`hicode`、`init`、`scope`、`tdd`、`review` 和 `release`。本轮重构保持入口名称和数量不变，只重写内部执行内容；这些 Skill 必须直接告诉 Coding Agent 做什么、按什么顺序做、何时停止和如何输出，不能只作为 `references/` 的索引。
_Avoid_: 把根目录 Skill 写成空壳引用、把 8 个细粒度 Skill 全部暴露为顶层入口、让用户在过多 Skill 名称中选择

**hicode 引导型总入口 Skill**:
`skills/hicode/SKILL.md` 是 hicode 的导诊入口，负责先判断目标项目是否已有 hicode 上下文资产；若没有 `AGENTS.md` / `CLAUDE.md`、`CONTEXT.md` 或项目上下文，应先引导用户进入 `init` 初始化，再路由到 `scope`、`tdd`、`review` 或 `release`。
_Avoid_: 未初始化就直接进入实现或 Review、默认全仓扫描、自动生成项目文件、把 plugin 安装和目标项目初始化混为一步

**hicode 初始化引导边界**:
`hicode` Skill 默认只诊断目标项目初始化状态、整理初始化输入并路由到 `init`；只有用户明确要求初始化并确认写入范围后，才由 `init` 协助生成或修改目标项目 `AGENTS.md`、`CLAUDE.md`、`CONTEXT.md`、`docs/PROJ_CONTEXT.md` 或规则文档。
_Avoid_: 无确认写文件、全仓扫描、读取敏感配置、把初始化建议当成真实初始化结果

**hicode 初始化场景 Skill**:
`skills/init/SKILL.md` 是 hicode 的独立目标项目初始化入口，仅在用户明确要求初始化目标项目时触发。它负责按当前 Coding Agent 平台检查或创建目标项目入口文件：Claude Code 使用 `CLAUDE.md`，OpenCode、Codex 和其他 Coding Agent 使用 `AGENTS.md`。随后初始化目标项目文档和 RULES 文档，并在用户确认代码扫描范围后使用 graphify 或等效代码图谱结果补充 `docs/PROJ_CONTEXT.md`。Agent、Skill、Rule、Template 和 Hook 等标准能力来自 hicode plugin，不复制到目标项目 `.hicode/`，不再执行 manifest/profile 选择性固化。它不替代 `hicode` 总入口的诊断和路由职责，也不由 plugin 安装动作自动触发。
_Avoid_: plugin 安装脚本、无确认写入目标项目、默认全仓扫描、生产数据扫描器、把代码图谱推断写成已确认业务事实
...
**hicode Agent 重构口径**:
本轮重构需要更新 `agents/` 中旧 `references/prompts/`、`references/skills/`、`references/gates/`、`references/schemas/`、`references/docs/` 和 guide 文件引用。Agent 仍保持短角色入口定位，只维护角色、触发条件、边界、委托流程、输出要求和安全红线；需要规则依据时只引用必要的 `references/rules/` 场景规则，必要指南要点应压缩写入 Agent 正文。
_Avoid_: 把 8 个 Agent 改成长规则文档、Agent 继续引用归档路径、Agent 默认读取全部 rules、Agent 替代 Skill 流程编排
...
**hicode Gate 归档策略**:
旧 `references/gates/` 不再作为当前独立资产层保留。迁移时应将门禁判定规则、准入条件、阻断建议项和风险提示项吸收到 `references/rules/` 的共享或场景规则；将门禁报告输出骨架迁入 `references/templates/`；原 Gate 文件整体进入 `archive/`。Skill 输出可以继续使用门禁建议结论，但不得把 Gate 当作独立执行资产或审批层。
...
**hicode 项目规则文档拆分策略**:
旧 `references/docs/` 中的项目规则文档，例如编码规范、测试指南、Review 规则和发布指南，迁移时应拆成两类：面向目标项目复制填写的模板版进入 `references/templates/project/`；hicode 当前执行需要的通用规则进入 `references/rules/shared/` 或对应场景规则。原文件进入 `archive/` 或拆解后移除当前路径，避免同一文件同时承担目标项目模板和 hicode 执行规则两种职责。
...
**Agent 单文件平铺结构**:
`agents/` 下的子 Agent 源资产采用单文件平铺结构，例如 `code-reviewer.md` 和 `java-reviewer.md`，并配套 `README.md` 与 `_template.md`。Agent 文件是短角色入口，不为每个 Agent 创建独立目录；详细规则和输出骨架分别由 `references/rules/` 与 `references/templates/` 承载，必要指南要点压缩进 Agent 正文。
...
**Agent 规则源引用口径**:
V3 重构后，hicode 子 Agent 只能引用必要的 `references/rules/` 场景规则作为详细规则源。Agent 文件只维护角色、触发、边界、降噪和安全红线，不重写详细检查规则，也不引用归档 Prompt、Gate、Schema、Guide 或旧细粒度 Skill。
_Avoid_: 为首批专项 Agent 复制长规则、在 Agent 中复制 review 规则全文、让 Agent 规则源指向归档路径
...
**目标项目入口轻量路由**:
`references/templates/project/AGENTS.md` 和 `references/templates/project/CLAUDE.md` 是目标项目第一入口模板，只维护上下文索引、核心流程路由、子 Agent 委托入口、安全边界和输出规范。它们可以列出核心场景的推荐 Skill、Agent 和 Rule，但不复制完整规则、降级表、冲突优先级或规则全文。
_Avoid_: 把目标项目入口写成长篇整合规范、在入口和规则目录维护两份完整映射、把所有 Skill/Rule 细则塞入 AGENTS.md
...
**hicode 初始化规划目录**:
原 `references/init/` 是 V2 历史设计中的目标项目初始化和可选固化清单源资产目录。当前重构后不再作为有效目录；目标项目初始化由 `skills/init/SKILL.md` 直接引导创建入口和项目文档，历史 manifest/profile 归档到 `archive/`。
...
**hicode init 代码图谱边界**:
`init` Skill 在目标项目已有代码库时，先进行轻量文件和构建证据检测，再在用户确认扫描范围后使用 graphify 生成代码结构证据，用于补充 `docs/PROJ_CONTEXT.md`。graphify 不可用时，应询问用户是否安装；用户拒绝或无法安装时，才降级为基于 `rg --files`、目录结构、构建文件和关键入口文件的轻量结构分析。代码图谱结果只能写为结构证据、推断和待确认项，不能写成已确认业务规则。
_Avoid_: 默认全仓扫描、扫描 `.env` 或密钥/生产配置、跳过用户确认安装 graphify、把图谱推断写成事实

**hicode Coding Agent Plugin**:
当前第一版指安装到 Claude Code 的 hicode 插件资产，用于让 Claude Code 获得 hicode 的命令、入口、能力场景 Skill 或后续初始化指引。该插件以仓库根目录作为 Claude Code plugin root，不等同于目标项目 `.hicode/` 运行目录，不在安装阶段扫描目标项目代码，不生成 `CLAUDE.md`、`AGENTS.md` 或项目上下文。OpenCode plugin 机制与 Claude Code plugin 机制不同，后续若适配应独立设计。
_Avoid_: 目标项目初始化结果、业务项目代码扫描器、生产环境插件、把平台插件安装和项目初始化混为一步

**hicode plugin 内部分层**:
当前第一版对外只维护一个 Claude Code 规范的 `hicode plugin`，以仓库根目录作为 plugin root；内部再区分入口层和能力层。入口层提供 hicode 定位、安全边界和后续初始化入口；能力层直接维护可调用的 hicode Skill、Agent、规则和流程入口，不再维护二级 plugin 副本。OpenCode plugin 机制与 Claude Code plugin 机制不同，不纳入第一版同一目录模型。
_Avoid_: 同时混用 Claude Code 与 OpenCode 的 plugin 规范、拆成多个对外 plugin 增加安装心智、替代目标项目初始化、全量默认加载所有资产、绕过人工确认或门禁、自动合并发布或操作生产
...
**hicode Plugin 安装器**:
仓库根目录 `install.sh`，用于把 hicode Coding Agent Plugin 安装到当前用户的 Claude Code。安装器只处理 Claude Code plugin 安装或注册，不执行目标项目初始化、不扫描代码、不生成项目级入口文件或 `.hicode/` 运行资产。
_Avoid_: 目标项目初始化脚本、代码扫描脚本、生产发布脚本、自动修改业务仓库的工具

**hicode Plugin 安装资产边界**:
`.claude-plugin/plugin.json` 和 `install.sh` 必须避免把本仓库项目管理文档、历史文档或归档材料安装为目标 Coding Agent 的运行资产。当前 plugin manifest 应只暴露根目录 `skills/` 等必要可调用入口；根目录 `docs/`、`archive/` 和本仓库进度/实施计划/ADR 不应作为目标 Coding Agent 默认安装或默认读取内容。
...
**`AGENTS.md`**:
单独写 `AGENTS.md` 时默认指根目录本仓库 Agent 入口规则；交付给目标项目使用的入口模板必须明确写作 `references/templates/project/AGENTS.md`。
