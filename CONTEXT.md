...
**hicode 项目模板加载边界**:
`references/templates/project/` 中的目标项目模板不随 plugin 默认加载，也不在安装时自动写入目标项目。当前 `hicode:init` 不再用这些模板自行生成 `CLAUDE.md` 或 `AGENTS.md`；入口文件缺失时只调用平台 `/init`。项目模板只保留为可追溯、可复制填写的支撑材料，是否使用必须由用户明确确认。
_Avoid_: plugin 安装时加载项目模板、非 init 任务默认读取项目模板、无确认写入目标项目、把项目模板等同于本仓库 `docs/`、绕过 `/init` 自行生成入口文件
...
**hicode 初始化引导边界**:
`hi` Skill 默认只诊断目标项目初始化状态、整理初始化输入并路由到 `hicode:init`。`init` 采用引导式初始化，遇到不确定或需要用户决策时一次只问一个问题；目标项目入口文件缺失时只调用当前 Agent 平台的 `/init` 命令生成 `CLAUDE.md` 或 `AGENTS.md`，不自行生成入口文件。
_Avoid_: 无确认写文件、全仓扫描、读取敏感配置、把初始化建议当成真实初始化结果、自行生成 `CLAUDE.md` 或 `AGENTS.md`

**hicode 初始化场景 Skill**:
`skills/init/SKILL.md` 是 hicode 的独立目标项目初始化入口，仅在用户明确要求初始化目标项目时触发。它负责检查目标项目入口文件是否存在；入口不存在时调用平台 `/init`，入口存在时只在已有文件基础上补充 rules 信息。随后根据当前项目适用性参考 `references/rules/`，在目标项目 `docs/rules/` 下生成对应规则，适用规则可以直接拷贝。规则初始化后询问用户是否扫描代码；用户同意后启动子 Agent 调用 graphify 梳理代码结构。Agent、Skill、Rule、Template 和 Hook 等标准能力来自 hicode plugin，不复制到目标项目 `.hicode/`，不再执行 manifest/profile 选择性固化。它不替代 `hi` 总入口的诊断和路由职责，也不由 plugin 安装动作自动触发。
_Avoid_: plugin 安装脚本、无确认写入目标项目、默认全仓扫描、生产数据扫描器、把代码图谱推断写成已确认业务事实、自行生成入口文件、默认创建整套项目文档
...
**hicode init 代码图谱边界**:
`init` Skill 在 rules 初始化完成后询问用户是否扫描代码。用户同意后，启动子 Agent 调用 graphify（`https://github.com/safishamsi/graphify`）生成代码结构证据；扫描前必须确认扫描范围和排除路径。graphify 不可用时，应询问用户是否允许安装或改用轻量只读目录梳理。代码图谱结果只能写为结构证据、推断和待确认项，不能写成已确认业务规则。
