...
**目标项目 Agent 入口模板**:
`references/target-project/AGENTS.md`，用于作为未来试点仓库或目标项目可复用的 Agent 第一入口模板。该模板应作为项目入口、上下文索引、工作流程路由、安全边界、Skill 调用说明和输出规范，不作为完整领域知识库。
_Avoid_: 根目录 AGENTS.md、本仓库工作规则、项目进度台账、长篇领域知识、完整历史缺陷库

**研发上下文文档**:
未来的 `references/docs/PROJ_CONTEXT.md`，用于沉淀 Harness 或试点仓库的模块结构、关键流程、接口、数据、约束和历史风险。
...
**hicode 根目录资产分层**:
根目录只把 Claude Code 可直接识别或 Coding Agent 会直接调用的入口资产做成一等目录，其中 `skills/` 是直接可调用 Skill，`agents/` 是子 Agent，`references/` 承载 docs、prompts、gates、schemas 和 examples 等按需支撑资产。
_Avoid_: 把 prompts、gates、schemas 和 docs 全部平铺到根目录、让 Skill 只引用旧 `harness-assets/`、把 references 当成默认全量上下文
...
**hicode 初始化场景 Skill**:
`skills/init/SKILL.md` 是 hicode 的独立目标项目初始化入口，仅在用户明确要求初始化目标项目时触发。它负责按当前 Coding Agent 平台检查或创建目标项目入口文件：Claude Code 使用 `CLAUDE.md`，OpenCode、Codex 和其他 Coding Agent 使用 `AGENTS.md`。随后初始化目标项目文档和 RULES 文档，并在用户确认代码扫描范围后使用 graphify 或等效代码图谱结果补充 `docs/PROJ_CONTEXT.md`。Agent、Prompt、Skill 等标准能力默认来自 hicode plugin，不复制到目标项目；只有用户明确要求离线固化、本地锁版本或项目内自定义时才创建 `.hicode/` 快照。它不替代 `hicode` 总入口的诊断和路由职责，也不由 plugin 安装动作自动触发。
...
**目标项目内部文档目录**:
`references/docs/`，用于保存未来试点仓库可使用的人可读知识、规范、上下文、ADR、流程和运营支撑模板。
_Avoid_: 根目录 docs/、项目管理文档目录、项目术语上下文

**Harness 运行资产源目录**:
`agents/`、`references/prompts/`、`references/skills/`、`references/gates/`、`references/schemas/` 和 `references/examples/` 等可见目录，用于维护 hicode plugin 内置能力和可选固化到目标项目 `.hicode/` 的子 Agent、Prompt、Skill、门禁、Schema 和示例资产。
_Avoid_: 根目录 `.hicode/`、目标项目运行目录、旧 `harness-assets/` 源资产目录
...
**Agent 单文件平铺结构**:
`agents/` 下的子 Agent 源资产采用单文件平铺结构，例如 `code-reviewer.md` 和 `java-reviewer.md`，并配套 `README.md` 与 `_template.md`。Agent 文件是短角色入口，不为每个 Agent 创建独立目录；详细规则、输出模板和支撑材料继续由 Prompt、Skill、门禁、Schema、示例和文档承载。
...
**Agent README 范围**:
`agents/README.md` 只维护子 Agent 目录规范、文件命名、frontmatter、模板结构、权限边界、Prompt 防护基线和 Agent 与 Prompt/Skill/Gate 的映射原则，不维护首批 8 个 Agent 的完整映射表。完整映射表应由后续 `references/docs/AGENT_PROMPT_INTEGRATION.md` 维护。
...
**Agent 整合规范边界**:
`references/docs/AGENT_PROMPT_INTEGRATION.md` 与 `references/docs/workflows/agent-delegation.md` 用于定义 Agent、Prompt、Skill、门禁、Schema 和输出模板之间的职责关系、引用关系、委托流程、冲突优先级和降级口径。该整合规范不更新目标项目入口模板，不创建初始化清单，不设计 Hook，不新增 Prompt、Skill、门禁或 Schema。
...
**目标项目入口轻量路由**:
`references/target-project/AGENTS.md` 是目标项目第一入口，只维护上下文索引、核心流程路由、子 Agent 委托入口、安全边界和输出规范。它可以列出 8 个核心场景的推荐 Agent、Prompt、Skill 和 Gate，但不复制 `AGENT_PROMPT_INTEGRATION.md` 的完整映射表、降级表、冲突优先级或规则全文。
...
目标项目初始化或可选固化 hicode 资产时的上下文分层策略。`DAILY` 表示应随目标项目日常会话默认可用的入口、当前技术栈规则和关键项目文档；在可选固化场景中也可标识核心 Prompt/Skill/Agent 和关键门禁。`LIBRARY` 表示保留为可检索、可按需调用的参考资产、专项规则、示例和扩展能力。该策略用于降低上下文噪音和误触发风险，不表示删除或降低资产重要性。
_Avoid_: 全量资产每次默认加载、把 LIBRARY 资产删除、凭偏好分类而不引用目标项目证据

**hicode 初始化规划目录**:
`references/init/` 是后续维护目标项目初始化和可选固化清单的源资产目录，用于描述项目入口、RULES 文档和项目上下文如何初始化到目标项目，以及 Agent、Prompt、Skill、门禁、Hook、Schema 和示例在用户明确要求时如何固化到目标项目 `.hicode/`。该目录可承载 profile、manifest 和 `DAILY/LIBRARY` 加载分层，不放真实运行副本、不替代 hicode plugin，也不由 plugin 安装动作自动执行。
_Avoid_: 把初始化清单放进人读 `docs/`、在本仓库维护根目录 `.hicode/`、把初始化规划写成真实初始化结果、安装 plugin 时自动初始化业务项目

**hicode manifest**:
hicode 选择性初始化中的资产清单，用于记录源资产、目标路径、加载分层、适用场景和依赖关系。manifest 描述规划，不是安装日志、执行结果或审批记录；在源资产尚未创建时只能保留规划占位，不编造虚假条目。
_Avoid_: 把 manifest 当作真实初始化证明、用 manifest 授权发布或合并、为尚未创建的 Hook 写虚假源资产

**hicode profile**:
hicode 选择性初始化中的内部资产组合，用于表达某类目标项目应初始化或保留哪些项目文档和可选固化 manifest 条目。`core` 表示完整轻量闭环；`java-insurance-core` 表示 Java 保险核心系统默认组合；`full-library` 表示全量可检索资产库或固化快照。profile 不应随意改写 manifest 条目的加载分层。
_Avoid_: 把 core 缩减为只有入口文件、把 full-library 理解为全量默认加载、让 profile 覆盖资产原始 load_tier 语义

**hicode init profile 自动选择**:
`init` Skill 执行目标项目初始化时，profile 是内部资产选择策略，不要求用户理解或手动选择。Agent 应根据目标项目入口文件、技术栈、目录结构、构建文件、代码语言和风险线索自动选择合适资产组合，并在输出中说明证据和写入范围；只有写入目标文件、扫描代码或覆盖已有内容需要用户确认。
...
**目标项目 Harness 运行目录**:
目标项目中的 `.hicode/` 是可选固化目录，只在用户明确要求离线固化、本地锁版本或项目内自定义 hicode 能力时，由初始化流程根据 hicode 根目录源资产和 `references/init/` 规划生成或复制；默认初始化不创建 `.hicode/`，不在本仓库作为源产出目录直接维护。
_Avoid_: references/prompts/、references/skills/、本仓库源资产目录
...
**Prompt 模板库**:
`references/prompts/` 下的 V1 场景化 Prompt 集合，用于约束 Agent 在需求、编码、测试、Review、提交和发布检查中的输入、处理规则、安全边界和输出格式；当前默认随 hicode plugin 作为标准能力源使用，只有用户明确要求固化快照时才复制到目标项目 `.hicode/prompts/`。
_Avoid_: Skill 实现、workflow 文档、自由聊天提示语

**Prompt 模板规范**:
`references/prompts/README.md` 和 `_template.md` 共同定义的 Prompt 通用结构、语言规则、安全红线和质量口径。
...
**大型代码库 Agent 工作方法论**:
面向多模块单体、Monorepo、多仓服务群或长期遗留系统的 hicode Agent 使用方法，强调实时代码导航、分层上下文、按需 Skill/Agent、局部命令、Hook 边界、LSP/MCP 工具边界和治理 Owner。该方法论沉淀在 `references/docs/LARGE_CODEBASE_AGENT_GUIDE.md`，作为按需读取的指导资产，不替代入口规则、项目上下文、Prompt、Skill、门禁或人工负责人确认。
_Avoid_: 全仓盲扫、把所有规则塞入 AGENTS.md、全量默认加载、让所有子 Agent 同时运行、用 Hook/MCP/LSP 替代测试或审批
...
**上下文捕获 Hook**:
`context-capture-hook` 是 V2 后续增强中的持续改进类 Hook，用于在 Agent 会话结束时提出 `AGENTS.md`、`docs/PROJ_CONTEXT.md`、子目录上下文文件、排除路径、局部命令和风险规则的更新建议。它不属于首批门禁 Hook，不关联 Gate，不产生 blocking，不自动写入长期上下文，必须等待 hicode Owner 或项目负责人确认后归并。
...
**`docs/`**:
单独写 `docs/` 时默认指根目录项目管理文档目录；交付给试点仓库使用的文档必须明确写作 `references/docs/`。

**`AGENTS.md`**:
单独写 `AGENTS.md` 时默认指根目录本仓库 Agent 入口规则；交付给试点仓库使用的入口模板必须明确写作 `references/target-project/AGENTS.md`。

**`.hicode/`**:
单独写 `.hicode/` 时默认指目标项目可选固化目录；默认项目初始化不创建该目录，只有用户明确要求离线固化、本地锁版本或项目内自定义 hicode 能力时才生成。本仓库源产出必须使用 `agents/`、`skills/`、`references/prompts/`、`references/skills/`、`references/gates/`、`references/schemas/` 等可见目录。
