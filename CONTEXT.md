...
This context defines the project language for building the hicode engineering system. It is a glossary only; implementation plans, templates, and detailed designs belong in their documented project locations.

## Language

**hicode 工程化体系**:
面向高严谨业务系统的 AI 辅助研发工程体系，用于规范 Agent 如何读取上下文、使用模板、执行检查和沉淀资产。
_Avoid_: 仅保险或金融核心系统、业务系统实现、单一 Prompt 集合

**高严谨业务系统**:
hicode 工程化体系的默认服务对象，指对业务规则严谨性、逻辑边界、交易或数据一致性、可审计性有高要求的系统；金融、保险、支付、清结算、账务、计费和订单履约等金融或类金融系统是重点场景，但不是唯一范围。
_Avoid_: 仅保险/金融核心系统、通用低风险应用、本仓库实现业务系统

**高严谨业务系统风险标准**:
后续 hicode 资产设计的默认风险基线，重点关注领域业务逻辑严谨性、金额与关键数值精度、交易与数据一致性、状态流转、幂等、并发、权限、审计、隐私、适用监管与合规、生产变更、回滚和发布准入。
_Avoid_: 仅按行业名称判定风险、普通 Web 应用风险标准、非生产演示标准、仅代码风格检查

**统一风险分级与场景矩阵**:
面向 Prompt、Skill 和门禁共用的风险口径，用 P0/P1/P2/P3 标识高严谨业务系统风险等级，并按需求评审、编码计划、TDD、代码审查、提交检查、核心场景测试和发布检查等场景映射必检风险。
_Avoid_: 每个资产重复定义不同风险等级、只罗列风险词但没有分级口径、忽略场景差异

**领域业务逻辑严谨性**:
目标系统的领域术语、规则、公式、状态、期间、权益、例外和不变量必须被精确理解、审查和验证；保险场景中的投保、批改、退保、核保、理赔、收付费和责任判断是其中一类重点实例。
_Avoid_: 只做代码风格检查、用通用规则替代具体领域规则、编造未确认业务规则

**需求草案**:
项目原始方案输入，当前指 `docs/研发 AI 工程化方案V1.1.md`。该文件定位为目标项目落地蓝图和 V1 需求来源，会持续调整，不作为 Agent 默认启动必读文件；只有工作包、冲突追溯或用户要求时才按需读取。
_Avoid_: 默认执行基准、已验收 Harness 资产

**目标项目落地蓝图**:
面向未来试点仓库或目标项目的 Harness 落地说明，描述目标、原则、能力边界、目标项目最终目录、试点运行和验收口径。
_Avoid_: 本仓库执行基准、单个资产详细设计

**执行基准**:
Agent 执行当前 Harness 工作时优先读取和对齐的文件组合。V3 简化重构期间，执行基准为 `AGENTS.md`、`CONTEXT.md` 与 `docs/V3_IMPLEMENTATION_PLAN.md`；V1/V2 计划仅作为历史追溯资料按需读取。
...
**目标项目 hicode 入口补充片段**:
重构后由 `skills/init/hicode-entry-section.md` 承载，用于补充到目标项目已有、由 Agent 可执行初始化能力生成或由用户手工初始化生成的 `AGENTS.md` 或 `CLAUDE.md`。非 Claude Code 平台主要使用 `AGENTS.md`，Claude Code 使用 `CLAUDE.md`；hicode 不再维护完整入口模板，只维护文档路径、单需求文档生命周期、Skill 路由、规则目录和安全边界等平台无关补充片段。旧 `references/target-project/` 和 `references/templates/` 不再作为当前有效目录。
_Avoid_: 根目录 AGENTS.md、本仓库工作规则、完整入口模板、长篇领域知识、完整历史缺陷库

**研发上下文文档**:
目标项目中的 `docs/PROJ_CONTEXT.md`，用于沉淀目标项目的模块结构、关键流程、接口、数据、约束和历史风险。重构后本仓库只在 `skills/init/PROJ_CONTEXT.md` 维护可复制填写的初始化模板，不再把 `references/docs/PROJ_CONTEXT.md` 或 `references/templates/project/PROJ_CONTEXT.md` 作为当前有效资产。
_Avoid_: 根目录 CONTEXT.md、项目术语表
...
**项目管理文档目录**:
根目录 `docs/`，用于保存需求源、实施计划、ADR、检查记录等本仓库管理文档。
_Avoid_: Harness 交付资产目录、试点仓库文档目录

**hicode 设计中心**:
仓库根目录是后续 hicode 资产设计、Claude Code plugin 发布和 Coding Agent 能力组织的中心，直接承载 `.claude-plugin/`、`skills/`、`agents/`、`hooks/` 和安装入口。
...
**hicode 根目录源资产**:
仓库根目录下直接维护的 hicode 一等源资产，包括 `skills/`、`agents/`、`hooks/`、`.claude-plugin/` 和 `install.sh`；这些资产是后续设计、安装和调用的主来源。
_Avoid_: `harness-assets/plugins/` 二级副本、只供 plugin 使用的临时快照、与旧 `harness-assets/` 源资产并行维护

**hicode 根目录资产分层**:
根目录只把 Claude Code 可直接识别或 Coding Agent 会直接调用的入口资产做成一等目录，其中 `skills/` 是直接可调用 Skill，`skills/init/coding_rules.md` 是 `hicode:init` 目标项目规则种子，`skills/<skill>/` 是各 Skill 具体模板文档的平铺目录，`agents/` 是子 Agent，`hooks/` 只承载当前 Hook 说明和目录索引，`archive/` 保存历史追溯材料。
_Avoid_: 把 prompts、gates、schemas 和 docs 全部平铺到根目录、让 Skill 只引用旧 `harness-assets/`、把 hooks 当成默认全量上下文、把 archive 当成当前规则源

**hicode Skill 本地运行资产**:
各场景 Skill 目录内用于目标项目执行的最小本地资产。`hicode:init` 可以读取 `skills/init/coding_rules.md` 作为创建或更新目标项目 `docs/rules/` 的种子规则；其他场景 Skill 不读取本仓库规则副本，只读取目标项目入口规则和 `docs/rules/`。各 Skill 的具体模板文档平铺在 Skill 根目录，只在目标项目文档缺失且需要创建草稿时使用；单需求生命周期规则写入目标项目入口，不在每个场景 Skill 下复制 `README.md`。
_Avoid_: 非 init Skill 必读本仓库 `coding_rules.md`、Skill 直接读取本仓库 Hook 说明替代目标项目规则、把模板当成目标项目事实、在非 init 场景读取项目级模板替代 `docs/DOMAIN_KNOWLEDGE.md` 或 `docs/PROJ_CONTEXT.md`、每个场景 Skill 维护重复生命周期 README

**hicode Agent 自包含共性规则口径**:
专业 Agent 的共性安全、权限、输出和停止条件写入各 Agent 正文第 2、7、8、10 节；Agent 不再通过共享运行目录读取共性规则或模板。模板骨架由对应场景 Skill 根目录中的具体模板文档或目标项目已有 `docs/features/` 报告结构承载。
_Avoid_: Agent 依赖共享运行目录、把模板当成目标项目事实、让 Agent 读取 Hook 说明作为运行默认上下文

**hicode 当前有效资产**:
hicode 当前执行、安装和 Skill/Agent 路由可引用的资产集合。重构后应只包括根目录 `skills/`、`skills/init/`、各 Skill 根目录具体模板文档、`agents/` 以及根目录 `hooks/` 等被明确保留的当前目录；归档资产不得再作为当前 Skill 的执行依据或默认检索来源。目标项目执行非 init Skill 时优先读取目标项目入口规则、`docs/rules/` 和本 Skill 根目录中的模板文档，避免跨 plugin 运行资产边界读取本仓库支撑说明。
...
**hicode init 规则种子**:
`skills/init/coding_rules.md`，用于 `hicode:init` 创建或更新目标项目 `docs/rules/`。它是当前唯一内置规则种子；`references/rules/` 不再作为当前维护源或运行依赖。
_Avoid_: 多套规则源并行、恢复 `references/rules/` 第二维护源、Prompt/Gate/Schema 独立一级目录继续承载当前规则、按旧资产类型换皮保留复杂度、规则散落在模板或归档区

**hicode 规则按需读取边界**:
`hicode:init` 可以读取 `skills/init/coding_rules.md` 生成目标项目规则文件；`scope`、`tdd`、`review`、`release` 在目标项目执行时只读取并遵守目标项目入口规则和 `docs/rules/`。专业 Agent 的共性安全、权限、输出和停止条件写入各 Agent 正文。Hook 说明和维护文档可以追溯 `skills/init/coding_rules.md` 与目标项目 `docs/rules/` 生成口径，但不读取旧 `references/rules/`。
_Avoid_: 默认读取全部规则、每个 Skill 都加载本仓库规则种子、Agent 为单一专项任务读取全库规则、把已删除的规则目录当成当前 interface

**hicode Markdown 结构化输出规则**:
当前用于约束 Skill、Agent、Review 和门禁类输出的 Markdown 规则，记录固定字段、稳定枚举、风险等级、建议结论、问题列表、证据记录和未执行验证原因。旧 JSON Schema 文件不再作为当前有效资产；仍有效的字段和枚举压缩进根目录 Skill、专业 Agent 或 `skills/init/coding_rules.md` 生成的目标项目规则中。
_Avoid_: 当前 Skill 读取 JSON Schema、为未接入自动校验的结构保留独立 JSON、每个场景自定义冲突字段、把建议结论写成审批状态

**hicode Skill 本地模板目录**:
各 Skill 根目录中的模板文档，用于目标项目文档缺失且需要创建草稿时提供结构化填写骨架。项目初始化模板位于 `skills/init/`，单需求和场景报告模板位于对应 `skills/scope/`、`skills/tdd/`、`skills/review/`、`skills/release/` 根目录；`references/templates/` 不再作为当前维护源。模板不承载执行规则全文，规则应进入 Skill/Agent 正文、`skills/init/coding_rules.md` 或目标项目 `docs/rules/`。
_Avoid_: 把模板写成规则库、把执行流程藏在模板里、让模板替代 Skill 或目标项目事实、把历史建设过程模板暴露为当前目标项目模板、按旧 docs/prompts/examples 类型散放模板
...
**目标项目 Feature 索引**:
目标项目 `docs/PROJ_CONTEXT.md` 中的历史需求定位表，用 `feature-id`、需求名称、状态、关联模块、文档路径、关键上下文摘要和风险等级连接 `docs/features/<feature-id>/` 下的单需求材料。它用于在用户输入不清晰时定位候选 feature、补充工作交接上下文，并让 `scope`、`tdd`、`review`、`release` 按需参考历史需求上下文；索引只保存可脱敏摘要和路径，不替代当前 `feature_context.md`、阶段报告或负责人确认。
_Avoid_: 把历史需求摘要写成当前需求事实、保存未脱敏客户或生产信息、替代 `docs/features/<feature-id>/` 详细材料、自动批准编码或发布

**目标项目文档目录分层**:
目标项目文档分为项目级共享文档、单需求特性文档和跨需求报告文档。项目级共享文档放在 `docs/` 和 `docs/adr/`，包括 `DOMAIN_KNOWLEDGE.md`、`PROJ_CONTEXT.md`、`docs/adr/` 和 `docs/rules/`；单需求特性文档放在 `docs/features/<feature-id>/`，包括 `feature_context.md`、`scope-plan.md`，以及复杂、高风险、需交接审计或用户明确要求时创建的 TDD 报告；简单 TDD 任务只保留代码、测试和对话验证结果，不生成阶段文档。Review 报告和 Release 报告不放到某个 feature 目录，统一放在 `doc/versions/`，文件名分别为 `review-report-<YYYYMMDD-HHmm>.md` 和 `release-report-<YYYYMMDD-HHmm>.md`，并按需引用一个或多个 feature 文档作为证据。入口文档 `CLAUDE.md` 或 `AGENTS.md` 必须说明这些路径和单需求文档生命周期。相关 Skill 读取事实时必须优先读取目标项目 `docs/`；只有目标文档缺失且需要创建草稿时，才读取本 Skill 内置模板。
_Avoid_: 把单需求文档平铺到 `docs/`、入口文件不说明文档路径、Skill 在缺少目标文档时自由生成无模板结构的文档

**单需求文档生命周期规则**:
`skills/init/hicode-entry-section.md` 中集中维护的单需求文档创建、更新、缺失材料处理、证据降级和阶段交接规则；`hicode:init` 将其写入目标项目 `AGENTS.md` 或 `CLAUDE.md`。`scope`、`tdd`、`review` 和 `release` 只保留本阶段动作，并在目标项目执行时读取目标项目入口中的 hicode section。
_Avoid_: 每个 Skill 复制一套 `docs/features/<feature-id>/` 写入规则、缺失材料处理口径不一致、把阶段报告写成审批结论、在场景 Skill 下维护重复 `README.md`

**hicode 项目模板加载边界**:
`hicode:init` 需要的项目模板放在 `skills/init/`，不随 plugin 安装自动写入目标项目；根目录 `hooks/` 不维护项目模板副本。当前 `hicode:init` 初始化入口文件时优先使用 Agent 可执行初始化能力；若目标平台只有用户手工 TUI `/init`，例如 OpenCode，则 Agent 不得声称已调用 `/init`，应提示用户可手工执行，并把“确认后生成最小 `AGENTS.md`”作为推荐继续路径。入口文件生成或存在后，hicode 只基于 `hicode-entry-section.md` 补充 hicode section，包含文档路径、单需求生命周期、Skill 路由、规则目录和安全边界。项目模板只在用户确认创建目标项目文档草稿时使用。
_Avoid_: plugin 安装时加载项目模板、非 init 任务默认读取项目模板、无确认写入目标项目、维护完整入口模板、把项目模板等同于本仓库 `docs/`、把用户手工 TUI 命令写成 Agent 已执行动作

**hicode Hook 目录**:
根目录 `hooks/`，用于维护当前有效的 Hook 行为说明、配置示例、触发条件、阻断建议和审计字段。Hook 目录不依赖旧 `references/init/manifests/hooks.json`，也不由 plugin 安装器自动启用；用户或目标平台需要启用 Hook 时，必须另行确认配置范围。当前 Hook 只能覆盖本地研发流程中的低风险检查和上下文捕获，不自动化生产发布、回滚、生产配置或生产数据相关动作。
_Avoid_: Hook 自动启用、Hook 连接生产环境、通过归档 manifest 启用 Hook、把 Hook 建议写成最终审批

**hicode Hook 目录契约**:
`hooks/README.md` 是当前 Hook 说明目录的短入口说明，只维护 Hook 行为目录、触发边界、禁止动作和人工适配规则。它不作为 docs/prompts/gates/schemas/examples/init/fine-grained skills 的索引，也不替代 Skill 或目标项目规则。
_Avoid_: 把 `hooks/README.md` 写成长篇规则库、继续列旧 references 目录作为当前资产、让 Hook README 替代 Skill 或 Rule

**hicode 场景 Skill**:
根目录 `skills/` 下直接维护的 Claude Code 可调用 Skill，当前固定保留 6 个入口：`hi`、`init`、`scope`、`tdd`、`review` 和 `release`。其中 `hi` 是总入口 Skill；`hicode:init`、`hicode:scope`、`hicode:tdd`、`hicode:review`、`hicode:release` 是用户可见的场景路由表达。这些 Skill 必须直接告诉 Coding Agent 做什么、按什么顺序做、何时停止和如何输出，不能只作为 Hook 或模板索引。
_Avoid_: 把根目录 Skill 写成空壳引用、把 8 个细粒度 Skill 全部暴露为顶层入口、让用户在过多 Skill 名称中选择

**hicode Scope 需求澄清与任务拆分**:
`skills/scope/SKILL.md` 承载需求到编码前的澄清、评审、设计树收敛和小任务拆分能力。它用于与用户确认目标、边界、模糊点、风险、验收标准、逻辑闭环、分支处理方案和后续 TDD 输入，并把过大的需求拆成可独立理解、验证和移交的小任务；它不生成业务实现代码，不替代 TDD、Review、发布检查或负责人审批。Scope 的核心产物包括单需求 `feature_context.md` 更新、`scope-plan.md`、必要 ADR 草稿或更新，以及经确认后可沉淀到 `DOMAIN_KNOWLEDGE.md` 和 `PROJ_CONTEXT.md` 的候选知识；`scope-plan.md` 统一承载需求准入、设计树方案和 TDD 任务计划。
_Avoid_: 直接编码入口、一次性生成大段实现代码、跳过需求确认的编码计划、最终审批结论

**Scope 设计树**:
`hicode:scope` 用于把用户输入或需求文档转换为可验证方案的树状结构。根节点是业务目标，主干节点描述正常逻辑闭环，分支节点描述异常、边界、状态、权限、幂等、回滚和外部依赖等处理方案，叶子节点映射到可独立交给 `hicode:tdd` 的任务。设计树只记录已确认事实、证据、待确认问题和建议方案，不把推断写成业务事实。
_Avoid_: 纯脑图、无证据方案、绕过需求确认直接拆任务、把未确认分支当成确定实现
...
**hicode 初始化引导边界**:
`hi` Skill 默认只诊断目标项目初始化状态、整理初始化输入并路由到 `hicode:init`。`init` 采用引导式初始化，遇到不确定或需要用户决策时一次只问一个问题；目标项目入口文件缺失时优先使用当前 Agent 可执行的初始化能力生成 `AGENTS.md` 或 `CLAUDE.md`。OpenCode `/init` 属于 TUI 内置命令，Agent 不能直接调用时提示用户可手工执行，并推荐在用户确认后生成最小入口文件。入口主体生成或存在后，hicode 只基于 `skills/init/hicode-entry-section.md` 补充 hicode section。
_Avoid_: 无确认写文件、全仓扫描、读取敏感配置、把初始化建议当成真实初始化结果、把 OpenCode TUI `/init` 当成 Agent 已执行命令

**hicode 初始化场景 Skill**:
`skills/init/SKILL.md` 是 hicode 的独立目标项目初始化入口，仅在用户明确要求初始化目标项目时触发。它负责检查目标项目入口文件是否存在；入口不存在时优先使用 Agent 可执行初始化能力，若只有用户手工 TUI `/init` 则不能代替用户执行，并以“用户确认后生成最小入口文件”为推荐继续路径；入口存在时只在已有文件基础上补充 hicode section。随后根据当前项目适用性读取本 Skill 目录下的规则种子，在目标项目 `docs/rules/` 下生成对应规则，适用规则可以直接拷贝。规则和项目文档初始化后评估代码复杂度；复杂度较高且用户同意时，再委托 graphify 梳理代码结构。Agent、Skill、Rule、Template 和 Hook 等标准能力来自 hicode plugin，不复制到目标项目 `.hicode/`，不再执行 manifest/profile 选择性固化。它不替代 `hi` 总入口的诊断和路由职责，也不由 plugin 安装动作自动触发。
_Avoid_: plugin 安装脚本、无确认写入目标项目、默认全仓扫描、生产数据扫描器、把代码图谱推断写成已确认业务事实、把用户手工 TUI `/init` 写成 Agent 已执行动作、默认创建整套项目文档

**Agent 可执行初始化能力**:
Agent 在当前运行环境中可以直接触发并确认结果的项目入口初始化能力，例如可调用工具、CLI 或平台暴露给 Agent 的等价初始化接口。OpenCode TUI `/init` 需要用户在终端中手工输入，不属于 Agent 可执行初始化能力。
_Avoid_: 用户手工 TUI 命令、安装器、hicode plugin 安装、目标项目业务初始化

**hicode 细粒度 Skill 细则**:
原 `references/skills/` 中维护的细粒度 Skill 规则源，例如需求评审、编码计划、TDD、辅助编码、代码审查、提交检查、核心场景测试和发布检查。重构后不再作为当前规则源目录；仍有效的执行规则应吸收到根目录场景 Skill、专业 Agent、`skills/init/coding_rules.md` 或目标项目 `docs/rules/`，历史形态进入 `archive/`。
...
**hicode Agent 重构口径**:
本轮重构需要更新 `agents/` 中旧 `references/prompts/`、`references/skills/`、`references/gates/`、`references/schemas/`、`references/docs/` 和 guide 文件引用。Agent 仍保持短角色入口定位，只维护角色、触发条件、边界、委托流程、输出要求和安全红线；共性安全、权限、输出和停止条件压缩写入 Agent 正文。
_Avoid_: 把 8 个 Agent 改成长规则文档、Agent 继续引用归档路径、Agent 默认读取全部 rules、Agent 引用不存在的规则子目录、Agent 替代 Skill 流程编排

**hicode Agent 共性规则**:
专业 Agent 正文中直接维护的短共性规则，包括 Prompt 防护、权限与受限命令、通用输出要求、安全红线和停止条件。Agent 文件仍只保留必要共性规则和角色差异，不复制场景 Skill 的完整执行细则。
_Avoid_: Agent 依赖共享运行目录、Agent 自行维护第二套场景流程、把共性规则拆回多个未确认规则文件
...
**hicode SkillOpt 管理侧离线评估优化链路**:
基于 SkillOpt 对 hicode Skill 进行离线评估、训练和优化的管理侧链路，用于构造脱敏评估数据、执行可复现 rollout、通过验证门禁比较候选 Skill，并把优化建议或候选 `best_skill.md` 作为人工审查输入。它不属于目标项目默认运行资产，不由 plugin 安装器自动启用，不扫描目标项目生产数据，也不替代 Skill Owner 对运行资产的审查和发布决策。
_Avoid_: 目标项目默认 Skill、自启动在线自优化、plugin 安装阶段训练、自动覆盖 `skills/`、使用未脱敏生产数据或客户信息、跳过人工审查直接采纳优化结果

**hicode SkillOpt 首批 Review 优化试点**:
SkillOpt 首阶段只把 `hicode:review` 作为训练优化对象，同时把 6 个 hicode Skill 纳入轻量触发边界和安全红线回归检查。该试点用于验证评估数据、evaluator、验证门禁和人工采纳流程是否成立，不代表一次性优化全部 Skill。
_Avoid_: 首批训练 6 个 Skill、把触发回归等同于 SkillOpt 训练、未验证 evaluator 前扩大到 init/tdd/release、用试点结果自动覆盖运行 Skill

**hicode SkillOpt 脱敏金标准样例**:
SkillOpt 首批评估训练使用的人工构造样例集合，包含脱敏 diff、需求证据、TDD 证据、配置/SQL/Java/高严谨领域风险线索、期望识别点、风险等级和禁止结论。它用于稳定 evaluator 和验证门禁，不直接采集真实会话、真实 MR、真实项目代码、生产日志、生产配置、客户信息或密钥。
_Avoid_: 未脱敏真实样例、生产数据、真实客户或保单材料、完整原始会话、真实 MR 原文、把样例当作真实试点效果

**hicode SkillOpt Review 混合 evaluator**:
首批 `hicode:review` 试点的评估器口径，以规则判分作为验证门禁主依据，检查必识别风险、禁止结论、风险等级、证据引用、安全红线和建议性输出枚举；LLM 只用于复核语义等价的风险识别，不作为单独放行依据。
_Avoid_: 纯主观 LLM 打分、只看格式不看 P0/P1 漏报、只看模型自评、把复核意见当最终审批、为提升分数降低安全红线

**hicode SkillOpt 管理侧资产目录**:
根目录 `skill-opt/`，用于集中维护 SkillOpt 离线评估优化相关的管理侧文档、脚本、脱敏样例规范、运行封装和本地输出约定。该目录不属于 hicode plugin 运行资产，不由 `install.sh` 安装到目标项目，不替代根目录 `docs/` 的项目管理文档定位，也不得被目标 Coding Agent 默认读取。
_Avoid_: `skillopt/`、`skill_opt/`、放入 `skills/` 或 `agents/`、由 plugin manifest 暴露、安装到目标项目、保存未脱敏真实样例

**hicode SkillOpt 本地输出目录**:
`skill-opt/outputs/` 是 SkillOpt 离线评估训练的本地输出位置，用于保存 rollout、checkpoint、日志、模型轨迹和候选 `best_skill.md`。该目录默认不提交到 Git；只提交目录说明文件，需要长期留存的结论应转写为脱敏运行摘要或人工采纳记录。
_Avoid_: 提交原始 rollout 或模型轨迹、把候选 `best_skill.md` 直接复制到 `skills/`、把本地输出当成已发布资产、保存生产数据或未脱敏输入

**hicode SkillOpt 安装边界健康检查**:
当前资产健康检查中用于约束 `skill-opt/` 的边界检查，确认 plugin manifest 和安装器不暴露或复制 `skill-opt/`，`skill-opt/outputs/` 默认被 Git 忽略且只保留说明文件，并检查 SkillOpt 管理侧文档不示例化密钥、生产凭证、未脱敏客户信息或生产数据。
_Avoid_: 把 `skill-opt/` 打进 plugin bundle、安装到目标项目、提交原始输出、用健康检查放宽安全红线

**hicode SkillOpt Python 依赖入口**:
根目录 `pyproject.toml`、`uv.lock` 和 `.python-version` 只作为 SkillOpt 管理侧 runner、adapter 和数据处理脚本的本地 Python 依赖入口；它们不表示 hicode 运行资产或目标项目初始化资产变成 Python 项目，也不得要求目标项目安装 SkillOpt。
_Avoid_: hicode 基础运行依赖、目标项目依赖、plugin 安装依赖、把模型凭证写入仓库配置、用 `main.py` 占位入口代表 hicode 应用程序

**hicode SkillOpt 依赖版本基线**:
P5 真实接入以 PyPI 发布版 `skillopt==0.1.0` 作为首版依赖基线，避免直接依赖上游 `main` 分支导致训练循环、adapter 注册或后端接口不可复现。后续只有在该版本无法满足外部 env 接入时，才单独评估 pin 到明确 Git commit。
_Avoid_: `git+https://github.com/microsoft/SkillOpt@main`、未锁版本、随上游 main 漂移、把依赖升级和 adapter 实现混在一起

**hicode SkillOpt 人工采纳补丁流程**:
SkillOpt 产出的候选 `best_skill.md` 或优化建议只能作为人工审查输入，必须先生成候选差异摘要，确认不冲突于 hicode 安全边界、V3 资产边界、中文文档风格、建议性输出枚举、Review 三轴审查和高严谨业务系统风险标准后，才能通过普通代码修改流程合入运行 Skill，并复跑健康检查和 held-out eval。
...
SearchQA 是 SkillOpt 上游内置的问答 benchmark，可作为理解 split directory、训练命令、输出目录和基线 smoke 的参考对象，但不是 `hicode:review` 的训练任务、验证门禁或候选采纳依据。`hicode:review` 的主训练链路应保持 Review 专用 adapter 和高严谨业务风险 evaluator 口径。
...
_Avoid_: 只写 Markdown 样例、无稳定 ID、无 split、无期望风险点、无禁止结论、样例中包含真实客户或生产材料

**hicode SkillOpt 派生 split 目录**:
SkillOpt 训练所需的 `train/val/test/items.json` split directory 是从已提交的 `skill-opt/data/review-golden/items.jsonl` 生成的运行期派生资产，默认位于 `skill-opt/outputs/<run-id>/split/`。它不作为手工维护的数据源提交，长期复现依赖同一 commit、同一 JSONL 和同一 run config 重建。
_Avoid_: 提交 `skill-opt/data/review-split/`、手工编辑派生 `items.json`、JSONL 与 split 双维护、把派生 split 当作 canonical 数据源

**hicode SkillOpt Review 风险分层切分**:
`hicode_review` 数据集的 train/val/test 切分口径，按安全红线、金额/状态/幂等、测试证据缺口、SQL/配置/脚本、Java/Spring 事务异常和无需求证据降级等风险场景分层覆盖。相似业务故事变体不得跨 train 和 test，以降低泄漏和虚高分数。
_Avoid_: 小样本随机切分、train/test 共享同一故事变体、val 缺少 P0/P1 或安全红线、只用 happy path 评估

**hicode Skill-Agent-Rule-Template 新职责关系**:
重构后，根目录 `skills/` 负责直接告诉 Coding Agent 在场景中做什么、按什么顺序做、何时停止和如何输出，并在各 Skill 根目录承载必要规则种子和模板文档；根目录 `agents/` 负责高风险或复杂场景下的专项角色委托；目标项目 `docs/rules/` 提供项目执行规则依据；根目录 `hooks/` 提供 Hook 行为说明和配置示例。Skill 可以读取本 Skill 根目录模板、目标项目规则和目标项目事实文档，也可以委托 Agent，但不得只引用旧细粒度 Skill 或把执行责任转交给归档资产。
...
**hicode Prompt 归档策略**:
旧 `references/prompts/` 不再作为当前资产类型保留。迁移后的有效内容应吸收到对应根目录 Skill、专业 Agent、`skills/init/coding_rules.md` 或各 Skill 根目录模板文档；原 Prompt 文件整体进入 `archive/` 作为历史追溯材料。迁移完成后，当前 Skill、Agent、Rule、Template 和 Hook 不得引用归档 Prompt。
_Avoid_: Prompt 继续作为当前规则源、Skill 为执行任务默认读取旧 Prompt、把 Prompt 原文复制进 Skill 导致冗长、丢失 Prompt 中仍有效的安全和风险规则

**hicode Gate 归档策略**:
旧 `references/gates/` 不再作为当前独立资产层保留。迁移后的门禁判定规则、准入条件、阻断建议项和风险提示项应吸收到根目录 Skill、专业 Agent、Hook 说明或 `skills/init/coding_rules.md` 生成的目标项目规则；门禁报告输出骨架应吸收到对应 Skill 根目录模板文档；原 Gate 文件整体进入 `archive/`。Skill 输出可以继续使用门禁建议结论，但不得把 Gate 当作独立执行资产或审批层。
...
**hicode 项目规则文档拆分策略**:
旧 `references/docs/` 中的项目规则文档，例如编码规范、测试指南、Review 规则和发布指南，迁移后应拆成两类：面向目标项目复制填写的模板版进入对应 Skill 根目录；hicode 当前执行需要的通用规则进入根目录 Skill、专业 Agent 或 `skills/init/coding_rules.md`。原文件进入 `archive/` 或拆解后移除当前路径，避免同一文件同时承担目标项目模板和 hicode 执行规则两种职责。
_Avoid_: 一个文件既是模板又是当前执行规则、Skill 默认读取目标项目模板当规则、项目模板缺少占位和维护说明、执行规则散落在归档文档

**hicode 简化重构实施策略**:
本轮 `references/` 与 `skills/` 简化重构采用先结构迁移、再内容重写、再运行资产收敛的策略。历史阶段曾创建 `archive/`、`references/rules/` 和 `references/templates/` 完成拆解；当前已进一步把规则种子和模板文档收敛到 `skills/<skill>/` 根目录，把 Hook 说明提升到根目录 `hooks/`，并删除 `references/`；最后通过健康检查验证路径、一致性和归档依赖。
...
_Avoid_: 继续用 V1 实施计划指导当前重构、未拆工作包就直接大规模改文件

**V3 工作包口径**:
V3 简化重构保留工作包编号、目标、输入、输出、依赖和验收标准，用于拆分可执行范围。工作包不再由独立动态状态文档维护，当前推进以用户最新指令和相关实施计划为准。
_Avoid_: 一次性完成全量重构后再补说明、未确认就扩大范围、用动态台账替代实施计划

**V3-P1-WP1 范围边界**:
V3 第一工作包只负责简化重构计划与入口规则更新，包括创建 `docs/V3_IMPLEMENTATION_PLAN.md`、更新根目录 `AGENTS.md` 执行基准和目录边界、整理已确认 `CONTEXT.md` 术语、保留 `docs/adr/0003-simplify-hicode-reference-assets-and-direct-skills.md` 决策记录。该工作包不移动 `references/` 文件、不创建 `archive/` 迁移内容、不重写 6 个 Skill。
_Avoid_: 计划尚未验收就开始目录迁移、在第一包混入 Skill 重写、把结构迁移和规则整理混在一起

**hicode Agent 入口规则同步要求**:
本轮简化重构必须同步更新根目录 `AGENTS.md` 的目录边界和工作规则。重构后 `AGENTS.md` 应把 `skills/`、`agents/`、`hooks/` 和根目录 `archive/` 定义为当前资产边界，并移除旧 `references/docs/`、`references/prompts/`、`references/skills/`、`references/gates/`、`references/schemas/`、`references/examples/`、`references/init/`、`references/target-project/`、`references/rules/`、`references/templates/` 和 `references/hooks/` 作为当前目录的说明。
_Avoid_: 后续 Agent 按旧目录边界继续新增资产、AGENTS.md 与 CONTEXT.md 冲突、归档目录被当作当前规则源

**目标项目内部文档目录**:
原 `references/docs/` 用于保存未来试点仓库可使用的人可读知识、规范、上下文、ADR、流程和运营支撑模板。重构后当前可复制填写的目标项目文档模板应进入对应 Skill 根目录，当前有效规则和流程应进入 Skill/Agent 正文、`skills/init/coding_rules.md` 或目标项目 `docs/rules/`，历史建设和运营支撑材料进入 `archive/`。
_Avoid_: 根目录 docs/、项目管理文档目录、项目术语上下文

**Harness 运行资产源目录**:
历史上指 `agents/`、`references/prompts/`、`references/skills/`、`references/gates/`、`references/schemas/` 和 `references/examples/` 等可见目录。重构后当前运行资产源目录应收敛为根目录 `skills/`、`agents/` 和 `hooks/`；不再维护可选固化到目标项目 `.hicode/` 的 manifest/profile 资产链路，也不再在 `references/` 下维护重复规则、模板或 Hook 源。
_Avoid_: 根目录 `.hicode/`、目标项目运行目录、旧 `harness-assets/` 源资产目录、归档目录作为当前运行资产源

**hicode 子 Agent**:
面向目标项目可委托执行的专门角色 Agent，源资产目录为 `agents/`，参考 ECC `agents/` 的 subagent delegation 模式，用于把规划、TDD、代码审查、安全审查、Java/SQL/高严谨领域业务专项审查、发布检查等高风险任务分配给稳定角色。子 Agent 负责角色边界、触发条件、审查纪律和输出优先级，不替代 Prompt、Skill、门禁、人工负责人或生产审批。
_Avoid_: 把所有 Skill 都改成 Agent、让子 Agent 自动合并或发布、用通用 Web Agent 替代高严谨业务专项 Agent

**Agent 与 Prompt 整合口径**:
hicode 子 Agent 作为可委托角色入口，定义角色、触发条件、工具权限、降噪纪律、证据要求和安全红线。`hicode:init` 通过本 Skill 目录下的规则种子创建目标项目 `docs/rules/`；其他场景 Skill 通过目标项目规则和本 Skill 目录下的模板承载运行依据；专业 Agent 的共性依据写入 Agent 正文。Agent 可以引用 Skill、目标项目规则和目标项目已有输出结构，但不复制 Prompt 全文，不维护第二套场景规则。
...
**Agent 单文件平铺结构**:
`agents/` 下的子 Agent 源资产采用单文件平铺结构，例如 `code-reviewer.md` 和 `java-reviewer.md`，并配套 `README.md` 与 `_template.md`。Agent 文件是短角色入口，不为每个 Agent 创建独立目录；共性安全、权限、输出和停止条件压缩进 Agent 正文，场景流程由对应 Skill 承载。
...
**Agent 规则源引用口径**:
V3 重构后，hicode 子 Agent 不读取仓库 `references/` 或共享运行目录作为默认上下文。Agent 文件只维护角色、触发、边界、共性安全规则、降噪和安全红线，不重写详细场景检查规则，也不引用归档 Prompt、Gate、Schema、Guide、旧细粒度 Skill 或不存在的规则子目录。
...
**专项审查 Agent 触发口径**:
`security-reviewer` 和 `java-reviewer` 是风险或技术栈触发的专项审查 Agent，不是所有变更默认必跑的全流程 Agent。`security-reviewer` 在权限、认证、鉴权、密钥、日志、脱敏、客户隐私、生产数据、外部接口、文件上传下载、加解密、审计、适用监管或安全红线相关变更中触发；`java-reviewer` 在 Java、Spring、MyBatis/JPA、消息、事务、SQL、批处理或高严谨交易后端变更中触发。两者不替代 `code-reviewer`、门禁或人工 Review。
...
**Agent 整合规范边界**:
V2 历史设计中，`references/docs/AGENT_PROMPT_INTEGRATION.md` 与 `references/docs/workflows/agent-delegation.md` 曾用于定义 Agent、Prompt、Skill、门禁、Schema 和输出模板之间的职责关系。V3 重构后，该整合规范不再作为当前规则入口；仍有效内容应拆入根目录 Skill、Agent 正文、`skills/init/coding_rules.md` 或目标项目 `docs/rules/`。
...
**Agent 委托流程口径**:
Agent 委托流程是当前 Skill 在高风险或复杂场景下选择专业子 Agent 的证据闭环流程，用于识别任务、检查安全红线、固定输入边界、选择 Agent、读取必要规则、执行分析或受控修改、记录验证动作、输出建议结论和上下文更新建议。该流程应体现在 Skill、Agent、`skills/init/coding_rules.md` 生成的目标项目规则和目标项目入口中，不再依赖旧独立 workflow 文件作为当前入口。
...
**目标项目入口轻量路由**:
`skills/init/hicode-entry-section.md` 是目标项目入口文件的 hicode 补充片段，只维护 hicode 文档路径、单需求文档生命周期、核心流程路由、安全边界和输出规范。目标项目入口主体优先由当前 Agent 可执行初始化能力生成；OpenCode TUI `/init` 等用户手工命令不能由 Agent 代替执行。hicode 不再维护完整 `AGENTS.md` 或 `CLAUDE.md` 模板，也不复制完整规则、降级表、冲突优先级或规则全文。
_Avoid_: 把 hicode 入口补充片段写成长篇整合规范、在入口和规则目录维护两份完整映射、把所有 Skill/Rule 细则塞入 AGENTS.md

**高风险优先委托口径**:
目标项目入口中，子 Agent 不是所有任务的强制默认入口。涉及高严谨领域业务规则、金额与关键数值、交易与数据一致性、状态、幂等、并发、权限、隐私、适用监管与合规、SQL、配置、发布、回滚、跨资产证据组织、专项审查、受控文件修改或门禁/Review/发布输入时，优先委托子 Agent；低风险、单一、只读、一次性报告或用户明确指定 Prompt/Skill 时，可以直接使用 Prompt/Skill。
...
**hicode 初始化规划目录**:
原 `references/init/` 是 V2 历史设计中的目标项目初始化和可选固化清单源资产目录。当前重构后不再作为有效目录；目标项目初始化由 `skills/init/SKILL.md` 引导使用 Agent 可执行初始化能力、补充或生成入口文件、生成 `docs/rules/` 项目规则，并在用户同意后启动子 Agent 调用 graphify 梳理代码结构。历史 manifest/profile 归档到 `archive/`。
...
**Harness 工程资产**:
本仓库负责建设和维护的资产集合，包括文档、Prompt、Skill、门禁规则、模板、示例和验收检查清单。
_Avoid_: 试点项目业务代码、人员排班、发布执行、组织运营计划

**Prompt 模板库**:
V1 历史设计中 `references/prompts/` 下的场景化 Prompt 集合，用于约束 Agent 在需求、编码、测试、Review、提交和发布检查中的输入、处理规则、安全边界和输出格式。当前重构后，Prompt 不再作为有效资产类型；仍有效内容应拆入根目录 Skill、专业 Agent、`skills/init/coding_rules.md` 或对应 Skill 根目录模板文档，原文件进入 `archive/`。
...
**代码实现前上下文清晰门槛**:
进入 TDD、辅助编码或代码实现前，编码计划 Skill 必须确认需求上下文已经足够清晰、明确、无歧义、无冲突，并能追溯到 `feature_context.md`、`scope-plan.md`、`PROJ_CONTEXT.md`、代码检索或负责人确认；未满足时只能输出澄清问题和上下文补齐动作，不得建议直接实现代码。
...
**Review 建议结论矩阵**:
`hicode:review` 使用的建议性结论选择口径，用 `BLOCKED` 表示 P0、安全红线或流程红线需要停止推进，用 `NEEDS_CONFIRMATION` 表示范围或关键证据不足以判断，用 `CONDITIONAL_RECOMMENDATION` 表示已识别 P1/P2 风险但可通过修复、补测或人工确认后继续，用 `NO_BLOCKING_FINDINGS` 表示已审范围内未发现 P0/P1 且证据足够。阻断建议项可以出现在 `CONDITIONAL_RECOMMENDATION` 中，不能把普通 P1 证据缺口一律升级为 `BLOCKED`；`NEEDS_CONFIRMATION` 不降低可见风险等级，完整代码或 CI 缺失也不必然阻止基于 diff 给出条件建议。
_Avoid_: 把建议结论写成审批结论、P1 普通缺口一律 BLOCKED、安全红线缺少停止推进表达、证据不足仍输出低风险结论、待确认就把高风险降级为 P2/P3、凭未确认 SQL 影响猜测 P0
...
**需求轴降级审查**:
双轴代码审查找不到需求、PRD、编码计划、TDD 或其他来源规格时，规范轴仍可继续，但需求轴必须标注无可用需求来源，不得编造需求一致性结论。最终结论不能是无条件通过；涉及高严谨业务系统高风险变更时，应提示补齐需求来源后重审。
...
**核心场景测试 Prompt**:
`references/prompts/core-scenario-test.md`，用于在测试阶段围绕高严谨业务系统变更整理必测场景、回归场景、边界、异常、测试数据、Mock/环境约束、自动化可行性、执行结果记录和缺口风险。它产出的核心场景测试报告可作为发布检查输入证据，但不替代测试负责人最终确认。
...
_Avoid_: 没有 Scope 准入证据仍建议通过、P0 澄清未关闭仍进入开发、核心业务规则缺失仍放行、把等价材料当作长期免除 `feature_context.md`
...
_Avoid_: 只凭主观体验判断资产有效、修改 Prompt/Skill 后不复测、没有高严谨业务风险样例

**V2 回归样例口径**:
V2 回归样例按场景集组织，不是单一 happy path 示例。每个样例文件应围绕一类 V2 能力覆盖多个验收场景，并采用可人工执行验收的 Markdown 结构，明确回归目标、适用资产、脱敏输入、执行步骤、期望输出要点、失败判定和禁止事项，用于验证 Agent 委托、选择性初始化、Hook advisory/blocking 边界、缺失资产降级、安全红线、生产越权拒绝和高严谨业务风险标准是否保持一致。
...
**`docs/`**:
单独写 `docs/` 时默认指根目录项目管理文档目录；目标项目事实文档必须明确写作目标项目 `docs/`，Skill 内置模板必须明确写作对应 `skills/<skill>/` 根目录文件。

**`AGENTS.md`**:
单独写 `AGENTS.md` 时默认指根目录本仓库 Agent 入口规则；目标项目实际入口文件写作目标项目根目录 `AGENTS.md`。hicode 当前只维护 `skills/init/hicode-entry-section.md` 作为可补充到目标项目入口文件的片段，不再维护完整 `AGENTS.md` 模板。

**`.hicode/`**:
单独写 `.hicode/` 时默认指 V2 历史设计中的目标项目可选固化目录。当前重构后，默认项目初始化不创建该目录，hicode 也不再复制 plugin 内置资产到目标项目 `.hicode/`；本仓库当前源资产必须使用 `skills/`、`agents/` 和 `hooks/`。

## Example Dialogue

开发 Agent：我开始任务前应该先读哪些内容？

项目负责人：默认先读 `AGENTS.md`、`CONTEXT.md` 和当前任务相关实施计划。V3 简化重构期间读取 `docs/V3_IMPLEMENTATION_PLAN.md`。需求草案不是默认必读，只有当前工作包需要追溯原始方案或用户要求时再读。
...
项目负责人：默认按高严谨业务系统服务对象设计。金融、保险及类金融系统是重点场景但不是唯一范围；即使本仓库不实现业务系统，Prompt、Skill、门禁、测试和发布资产也要覆盖领域业务逻辑严谨性、金额与关键数值、交易与数据一致性、状态流转、幂等、并发、权限、审计、隐私、适用监管与合规、生产变更和回滚等高风险点。

开发 Agent：我应该如何稳定引用一个工作包？

项目负责人：使用工作包编号，格式为 `P<阶段号>-WP<序号>`。当前是否推进某个工作包，以用户最新指令和相关实施计划为准。
