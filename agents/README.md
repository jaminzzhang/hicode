...
5. 不为每个 Agent 创建独立目录；详细规则和输出模板由根目录 `skills/`、`references/rules/` 和 `references/templates/` 承载。
...
每个 Agent 必须引用 `references/rules/coding_rules.md` 中的 Agent 共性规则，不在 Agent 文件内复制 Prompt 防护、权限与受限命令、通用输出要求、安全红线和停止条件全文。
...
Agent 可以引用 Skill、Rule 和 Template，但不得复制规则全文，不得维护第二套执行细则。涉及共性安全、权限、输出和停止条件时，以 `references/rules/coding_rules.md` 为准。
...
所有 Agent 必须遵守 `references/rules/coding_rules.md` 中的 Agent 共性规则。安全红线、权限边界、受限命令、通用输出要求和停止条件只在规则文件维护，Agent README 不复制全文。
