...
5. 不为每个 Agent 创建独立目录；场景流程由根目录 `skills/` 承载，Agent 共性安全、权限、输出和停止条件写入各 Agent 正文。
...
每个 Agent 必须在正文中包含短共性规则，覆盖 Prompt 防护、权限与受限命令、通用输出要求、安全红线和停止条件。
...
| Rule | 目标项目规则、风险基线、场景检查点和停止条件 |
| Template | 目标项目草稿文档、报告骨架、字段结构和可验收输出格式 |

Agent 可以引用 Skill、目标项目 Rule 和目标项目已有 Template/报告结构，但不得维护第二套场景执行细则。涉及共性安全、权限、输出和停止条件时，以 Agent 正文第 2、7、8、10 节为准。
...
所有 Agent 必须在正文中保留 Agent 共性规则。安全红线、权限边界、受限命令、通用输出要求和停止条件由各 Agent 的第 2、7、8、10 节承载。
