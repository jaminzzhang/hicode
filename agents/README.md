...
4. 工具权限、模型建议和 Hook 配置后续由 `references/init/` 或目标平台适配层处理。
...
V2-P1-WP1 的 README 只维护映射原则，不维护首批 8 个 Agent 的完整映射表。完整映射表由 `references/docs/AGENT_PROMPT_INTEGRATION.md` 维护。
