...
当前平台可用 hicode 子 Agent，且变更复杂或命中专项风险时，按需委托：

1. `code-reviewer`：通用代码审查、需求轴、规范轴、证据轴和合并前证据。
2. `security-reviewer`：权限、认证、密钥、日志、隐私、生产数据、审计和监管风险。
3. `java-reviewer`：Java/Spring、事务、异常、SQL、批处理、消息和保险核心后端风险。
...
`NO_BLOCKING_FINDINGS` 只表示已审范围内未发现阻断项；`CONDITIONAL_RECOMMENDATION` 表示需要补证据、修复或人工确认后再继续。二者都不是合并、审批或发布许可。
