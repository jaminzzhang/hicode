...
1. `AGENTS.md` 或 `CLAUDE.md`：目标项目 Coding Agent 入口。
2. `CONTEXT.md`：目标项目术语和概念边界。
3. `docs/PROJ_CONTEXT.md`：项目结构、模块、流程、接口、数据和历史风险上下文。
4. `.hicode/`：按 `references/init/` profile 规划安装的 hicode 运行资产。
...
2. 需求不清、范围不清、编码前计划或准入证据不足时，转 `hicode:scope`。
3. 需要测试先行、TDD、修复复现、受控实现或辅助编码时，转 `hicode:tdd`。
4. 需要代码审查、提交检查、安全/Java/SQL 专项审查或合并证据时，转 `hicode:review`。
5. 需要核心场景测试、发布材料、回滚、生产变更风险或发布准入证据时，转 `hicode:release`。
6. 用户明确要求初始化目标项目 hicode 时，先说明 plugin 安装与项目初始化不同，再按需读取 `references/init/`。
