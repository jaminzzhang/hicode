...
4. `docs/rules/`、`docs/DOMAIN_KNOWLEDGE.md`、`docs/PROJ_CONTEXT.md`、`docs/adr/` 等项目级文档路径明确，且 `PROJ_CONTEXT.md` 可承载 Feature 索引。
...
3. hicode section 应说明项目文档路径、`docs/rules/`、Feature 索引、、单需求目录和推荐 hicode Skill。
...
2. 缺少 `PROJ_CONTEXT.md` 时，先读取模板，再写入已确认项目定位、Feature 索引、模块结构、核心流程、接口依赖和历史风险；未知内容写 `待确认`。
3. 已存在 `docs/features/<feature-id>/` 且用户确认可纳入交接上下文时，只在 Feature 索引中写路径、状态和脱敏摘要；不得把历史 feature 摘要写成本次需求事实。
4. `docs/adr/` 可以只创建目录。只有存在难逆、意外且有真实取舍的决策时，才基于 `ADR-template.md` 生成 ADR 草稿。
5. 单需求文档目录统一使用 `docs/features/<feature-id>/`，入口 hicode section 应指向该路径。
