---
description: 初始化目标后端项目的 hicode 使用入口，优先调用当前 Coding 平台 /init 生成 AGENTS.md 或 CLAUDE.md，再补充 hicode section、项目规则、上下文文档，并按需建议 graphify 代码结构扫描。Use when 用户要求初始化 hicode、补齐 AGENTS.md/CLAUDE.md、创建 docs/rules、建立项目上下文，或准备目标项目进入 hicode 工作流。
---
...
2. `../../references/templates/project/hicode-entry-section.md`
3. `../../references/templates/project/DOMAIN_KNOWLEDGE.md`
4. `../../references/templates/project/PROJ_CONTEXT.md`
5. `../../references/templates/project/ADR-template.md`
...
3. 若入口缺失，优先调用当前平台 `/init`；若 `/init` 不可用，询问用户是否允许生成最小入口。
...
1. 优先调用当前 Coding 平台 `/init` 或等价初始化能力生成入口主体。
2. 如果当前平台不支持 `/init`，只能在用户确认后生成最小入口文件。
3. 最小入口文件只能包含标题、项目占位信息和 hicode section，不得手写一份完整平台入口来替代 `/init`。

入口文件已存在时：

1. 保留用户已有内容和平台生成内容。
2. 基于 `../../references/templates/project/hicode-entry-section.md` 补充或刷新 hicode section。
...
2. 适用且无需改写的规则可以从 `references/rules/` 复制。
