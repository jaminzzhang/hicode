...
本目录维护 hicode 面向 Claude Code 的原生 plugin 安装资产。

当前第一版只支持 Claude Code plugin 规范。OpenCode plugin 机制与 Claude Code plugin 机制不同，不纳入本目录第一版交付。
...
```bash
./harness-assets/plugins/install.sh --dry-run
./harness-assets/plugins/install.sh --yes
```
...
2. OpenCode Plugins：`https://opencode.ai/docs/zh-cn/plugins/`，仅作后续独立适配参考，不混入本目录第一版。
