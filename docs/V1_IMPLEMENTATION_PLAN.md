...
工作包是否启动、暂缓或验收，以用户最新指令和本计划中的目标、输出、依赖、验收标准为准。

## 5. 阶段总览

| 阶段 | 名称 | 目标 | 主要交付物 |
|---|---|---|---|
| P1 | 项目入口与计划机制 | 确保每次开发 Agent 都能了解项目定位、边界和实施计划 | `AGENTS.md`、`CONTEXT.md`、`docs/V1_IMPLEMENTATION_PLAN.md` |
...
目标：沉淀金融保险领域术语、核心业务域和高风险规则的初版结构。
...
建议每次只启动一个工作包，完成后根据用户最新指令决定是否进入下一个工作包。
